...
void solve() {
        ...
        std::set<std::array<int, 5>> with_cheats_distinct;
        for (int i = 0; i < N; ++i) for (int j = 0; j < M; ++j) if (grid[i][j] != '#') {
                for (int cheats = 2; cheats <= 20; ++ cheats) {
                        for (int x = 0, y = 0; x <= cheats; ++x) {
                                y = cheats - x;

                                for (int dx = -1; dx <= 1; ++dx) for (int dy = -1; dy <= 1; ++dy) if (std::abs(dx) * std::abs(dy) == 1) {
                                        int nx = i + dx * x, ny = j + dy * y;
                                        if (nx < 0 || ny < 0 || nx >= N || ny >= M || grid[nx][ny] == '#') continue;

                                        for (int t = 0, answer = INF; t < 2; ++t) {
                                                if (distance[nx][ny][t] == INF || distance[i][j][!t] == INF || nx > i || (nx == i && ny > j)) continue;
                                                answer = distance[nx][ny][t] + distance[i][j][!t] + cheats;

                                                if (answer >= without_cheats) continue;

                                                if (t) {
                                                        with_cheats_distinct.insert({i, j, nx, ny, answer});
                                                } else {
                                                        with_cheats_distinct.insert({nx, ny, i, j, answer});
                                                }
                                        }
                                }
                        }
        ...
        for (const auto& x : with_cheats_distinct) with_cheats.push_back(x[4]);
...
int main() {
        ...
        TimeCalculate timer;
        solve();
        timer.end(); // Execution Time: 21.086220595000000s
        return 0;
}
